  ...
  const act =
    async (
      ...
      if (
        ...
        setError(
          "Please write a clear reason so the farmer receives a useful notice."
        );
      ...
      if (
        action ===
          "remove" &&
        !window.confirm(
          `Remove “${selected.title}” from AgriNova?`
        )
      ) {
        return;
      }
      ...
      try {
        ...
        if (
          ...
          await marketplaceService.moderateProduct(
            selected._id,
            reason.trim()
          );
        } else if (
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              "disabled" ? (
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50"
                >
...
function FulfillmentTab() {
    ...
    useCallback(
      async () => {
        try {
          ...
          const result =
            await marketplaceService.getAdminOrders(
              {
                page,
                limit: 20,
          ...
          setError(
            err instanceof
              Error
              ? err.message
              : "Unable to load marketplace fulfillment."
          );
        } finally {
          setLoading(
            false
  ...
  const advance =
    async (
      ...
      const key =
        `${order._id}:${fulfillment.sellerId}`;
      ...
      if (
        ...
        setError(
          "Enter the assigned collection agent name before marking items picked up."
        );
      ...
      try {
        ...
        await marketplaceService.updateAdminFulfillment(
          order._id,
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <button
          ...
          className="rounded-xl border p-2.5 text-slate-600 hover:bg-slate-50"
        >
          <RefreshCw className="h-4 w-4" />
        </button>
      </div>
                ...
                >
                  ...
                  {next && (
                    <button
                      ...
                      className="mt-4 inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50"
                    >

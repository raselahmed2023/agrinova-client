...
export default function ExpertRegisterPage() {
  ...
  const imageStoragePurpose = "expert-registration";
  ...
  useEffect(() => {
    ...
    setImagePreview(newest.dataUrl);
  }, []);
  ...
  const handleImageChange = async (
    ...
    if (!file || isUploadingImage || isSubmitting) return;
    ...
    try {
      ...
      if (result.source === "remote") {
        ...
        const oldFallbacks = listStoredLocalImages(imageStoragePurpose);
        for (const item of oldFallbacks) {
          removeStoredLocalImage(item.localKey);
        }
      ...
      setUploadedAvatarUrl(null);
      setImagePreview(stored.dataUrl);
    } catch (err: any) {
  ...
  const retryLocalAvatar = async () => {
    if (!localAvatar || isUploadingImage || isSubmitting) return;
    ...
    try {
      ...
      setLocalAvatar(null);
      setUploadedAvatarUrl(remoteUrl);
  ...
  const onSubmit = async (data: ExpertRegisterInput) => {
    ...
    let avatarUrl = uploadedAvatarUrl || DEFAULT_AVATAR;

    if (localAvatar) {
      try {
        setIsUploadingImage(true);
        avatarUrl = await retryStoredImage(localAvatar.localKey);
        setLocalAvatar(null);
        ...
        setImageError(
          err?.message ||
            "Your photo is saved safely in this browser, but the image service is still unavailable. Please retry before submitting your application."
        );
        return;
      } finally {
        setIsUploadingImage(false);
  ...
  const hasCustomImage = Boolean(uploadedAvatarUrl || localAvatar);
      ...
      <div className="flex w-full flex-col justify-center px-4 py-8 sm:px-8 md:px-12 lg:w-1/2 lg:px-12 xl:px-20 overflow-y-auto">
        <div className="mx-auto w-full max-w-lg space-y-5 my-auto py-2">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
            ...
            <div
              className={`rounded-2xl border bg-slate-50/60 p-4 transition ${
                localAvatar
                  ? "border-amber-300"
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                ...
                <div className="relative group shrink-0">
                  ...
                  {localAvatar && !isUploadingImage && (
                    <span className="absolute -left-2 -top-2 z-20 inline-flex items-center gap-1 rounded-full bg-amber-500 px-2 py-0.5 text-[9px] font-bold uppercase text-white shadow">
                ...
                <div className="flex-1 text-center sm:text-left space-y-1.5">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight">
                    Upload a clear photo (JPG, PNG, WEBP, max 8MB). If skipped, our default avatar will be saved.
                  </p>
                  ...
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 pt-1">
                    ...
                    {localAvatar && (
                      <button
                        ...
                        disabled={isUploadingImage || isSubmitting}
                        className="inline-flex items-center gap-1 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-800 hover:bg-amber-100 transition disabled:opacity-50"
                      >
                    ...
                    {hasCustomImage && (
                      <button
                        ...
                        disabled={isUploadingImage || isSubmitting}
                        className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium text-rose-600 hover:bg-rose-50 transition disabled:opacity-50"
                      >
                  ...
                  {localAvatar && (
                    <p className="text-xs text-amber-700 font-medium pt-0.5">
                      Image service is unavailable. Your photo is saved safely in this browser and will be retried before submission.
                    </p>
                  )}

...
export default function AddFarmDrawer({ isOpen, onClose, onSuccess }: AddFarmDrawerProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.soilType) newErrors.soilType = 'Select a soil type';
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!apiKey) {
      setUploadError('API Key is missing');
      return;
    }
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/farms`, {
        ...
        body: JSON.stringify({
          ...
          coverImage: formData.coverImage.trim() !== '' ? formData.coverImage : undefined,
      ...
      if (data.success) {
        ...
        setErrors((prev) => ({ ...prev, form: data.message || 'Failed to create farm' }));
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm transition-opacity">
      <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col justify-between animate-in slide-in-from-right duration-300">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-xl font-bold text-slate-900">Add New Farm</h2>
        ...
        <form onSubmit={handleSubmit} noValidate className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border rounded-lg p-2.5 text-sm focus:outline-none transition ${
                errors.name ? 'border-red-500 focus:ring-2 focus:ring-red-200' : 'border-slate-200 focus:ring-2 focus:ring-emerald-500'
              }`}
            />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              {errors.division && <p className="text-[11px] text-red-500 mt-1">{errors.division}</p>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (errors.landArea) setErrors((prev) => ({ ...prev, landArea: '' }));
                }}
                className={`w-full border rounded-lg p-2.5 text-sm focus:outline-none transition ${
                  errors.landArea ? 'border-red-500 focus:ring-2 focus:ring-red-200' : 'border-slate-200 focus:ring-2 focus:ring-emerald-500'
                }`}
              />
              {errors.landArea && <p className="text-[11px] text-red-500 mt-1">{errors.landArea}</p>}
            </div>
          ...
          <div>
            ...
            <select
              ...
              className={`w-full border rounded-lg p-2.5 text-sm bg-white transition ${
                errors.soilType ? 'border-red-500' : 'border-slate-200'
              }`}
            >
              <option value="">Select dominant soil type...</option>
              <option value="Loamy">Loamy</option>
              <option value="Clay">Clay</option>
              <option value="Sandy">Sandy</option>
              <option value="Silt">Silt</option>
            </select>
            {errors.soilType && <p className="text-[11px] text-red-500 mt-1">{errors.soilType}</p>}
          </div>
          ...
          {/* Cover Image */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Cover Image (Optional)</label>
            <label className="border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 transition relative">
              {uploadingImg ? (
                ...
                <Image src={formData.coverImage} alt="Preview" fill className="object-cover rounded-lg" />
              ) : (
                <>
                  <UploadCloud className="w-8 h-8 text-slate-400 mb-2" />
                  <span className="text-xs text-slate-500 text-center">
                    Click to upload SVG, PNG, JPG (max. 5MB)
                  </span>
                </>
              )}
